...
class Pbl_kuis extends CI_Controller {
  ...
  {
    $data = $this->Pbl_tahap2_model->get_questions_by_quiz_id($quiz_id);
    $this->output
  ...
  public function save_quiz_question()
  {
    ...
    if ($id) {
    	$getQuestion = $this->Pbl_tahap2_model->get_questions_by_id($id);
      if (!$getQuestion) {
        echo json_encode(['status'=>'error','message'=>'Soal Kuis tidak ada!', 'csrf_hash' => $this->security->get_csrf_hash()]);
        return;
      }

      $this->Pbl_tahap2_model->update_quiz_question($id, $payload);
      $msg = 'Pertanyaan diperbarui.';
    } else {
      $payload['id'] = generate_ulid();
      $this->Pbl_tahap2_model->insert_quiz_question($payload);
  ...
  public function delete_quiz_question()
  {
    ...
    $this->Pbl_tahap2_model->delete_quiz_question($id);
  ...
  public function export_quiz($quiz_id)
  {
    // 1. Ambil data
    $quiz = $this->Pbl_tahap2_model->get_quiz_by_id($quiz_id);
    $questions = $this->Pbl_tahap2_model->get_questions_by_quiz_id($quiz_id);
  ...
  public function import_quiz()
  {
    ...
    if (!empty($batch_data)) {
      $this->Pbl_tahap2_model->insert_quiz_question_batch($batch_data);
  ...
  public function panduan_tahap2($class_id = null)
  {
    $data['user'] = $this->session->userdata();
    $data['url_name'] = 'guru';
    ...
    $this->load->view('templates/header', $data);
    $this->load->view('guru/panduan/tahap2_organisasi', $data);
    $this->load->view('templates/footer');
  }
